...
//Changes were made to work with a two button mouse
...
{
    int c_menu, p_menu, f_menu, r_menu; //added r_menu
    ...
    p_menu = glutCreateMenu(pixel_menu); //same as color_menu
    glutAddMenuEntry("increase pixel size", 1);
    glutAddMenuEntry("decrease pixel size", 2);
    f_menu = glutCreateMenu(fill_menu); //same as color_menu
    glutAddMenuEntry("fill on", 1);
    glutAddMenuEntry("fill off", 2);
	r_menu = glutCreateMenu(right_menu); //formerly attached to right button
	...
	glutAttachMenu(GLUT_RIGHT_BUTTON); //changed from MIDDLE
    myinit ();
