...
//Load a TGA image
//Author Andy Rustad- slightly modified
...
void tgaGetAPacket ( int &current_byte, image_t *p, FILE *file )
{
   ...
   if ( packet_header&0x80 )  // RLE packet
   {
      ...
      if ( p->info.components == 1 )  // Special optimized case :)
      {
...
void tgaGetImageHeader ( FILE *file, tgaHeader_t *info )
{
   /*   Stupid byte alignment means that we have to fread each field
        ...
        1 as it should be.  Grrr.  Gotta love optimising compilers - there's a pragma
...
int tgaLoadTheImage ( char *file_name, image_t *p, tgaFLAG mode )
{
   ...
   {
      case 1 :
         tgaError ( "8-bit color no longer supported", file_name, file, p );
         return 0;
...
GLuint tgaLoadAndBind ( char *file_name, tgaFLAG mode ) //this handles only one texture
{
   GLuint   texture_id;
   image_t  *p;
