   ...
   int sumDC(int lo, int hi) 
   {
      ...
      if (hi-lo < SEQUENTIAL_CUTOFF) {
   	 ...
   	 Sum *right = new Sum(arr, (hi-lo)/2);
         ...
         task_group g;
         g.run([&] { LeftAns = left->sumDC(lo,(hi+lo)/2); } );
         g.run([&] { RightAns = right->sumDC((hi+lo)/2,hi); } );
	 g.wait();
...
int main()
{
    ...
    cout << endl << "Computing sum of with Divide & Conquer class (parallel TBB task group) " << endl;
